...
(function(){
  ...
  function getOrCreatePeer(otherUserId){
    ...
    if(!pc){
      ...
      pc.ontrack = function(e){
        ...
        // muted:true — browsers routinely block autoplay of a non-muted
        // <video> that isn't started from a direct click, and an incoming
        // WebRTC track never counts as one. Muted autoplay is reliable
        // everywhere; unmuting would need its own "tap to unmute" control.
        if(seatId) window.BunkerUI.setSeatVideo(seatId, e.streams[0], true);
      };
    }

    // Attach local tracks whenever they're available — this runs both for
    // a brand-new connection AND for one that already existed (e.g. it was
    // opened earlier by the other side before my camera was on). Checking
    // existing senders keeps this safe to call repeatedly.
    if(localStream){
  ...
  function setupRoomChannel(){
    ...
    roomChannel.on('presence', { event: 'sync' }, function(){
      ...
      var newcomers = Object.keys(updated).filter(function(uid){
        return uid !== myUserId && !knownPeerIds[uid];
      });

      seatByUser = updated;
      Object.keys(updated).forEach(function(uid){ knownPeerIds[uid] = true; });

      if(cameraActive && newcomers.length){
        console.log('[Bunker] new peer(s) while camera is on, offering:', newcomers);
        newcomers.forEach(offerTo);
      }
    });
  ...
  function startCamera(seatId){
    // video only — no microphone requested. getUserMedia rejects the
    // WHOLE call if any one of several requested media types is denied,
    // so asking for audio here (which nothing plays anyway, see the
    // muted:true on the receiving side) was silently breaking the camera
    // whenever mic permission wasn't granted.
