/*
  ...
  Новые поля — нужно один раз выполнить в Supabase SQL Editor:
    ...
    alter table seats add column raided_for text;
    alter table seats add column eliminated text;

    alter table seats replica identity full;

  АВАТАРКИ — раньше картинка целиком (base64, часто сотни КБ) лежала
  прямо в строке seats. С replica identity full каждое изменение этой
  строки стало гонять по realtime-каналу аватарку целиком, а у Supabase
  Realtime есть лимит размера сообщения — если картинка большая,
  обновление тихо не долетает вообще (ни другим, ни самому себе при
  следующей пересинхронизации). Правильное решение — не хранить
  картинку в самой строке. Теперь аватарка загружается в Supabase
  Storage, а в базе лежит только маленькая ссылка на неё.

  Один раз выполнить в SQL Editor — создаёт публичный бакет и открытые
  политики (без авторизации, как и у остальной базы):
...
(function(){
  ...
  function patchToRow(seatId, patch){
    ...
    if('raidedFor' in patch) row.raided_for = patch.raidedFor || null;
    if('eliminated' in patch) row.eliminated = patch.eliminated || null;
  ...
  function subscribeToSeatChanges(){
    client.channel('seats-db-changes')
      ...
      .subscribe(function(status){
        // Fires on the first successful subscribe AND on every automatic
        // reconnect after a dropped connection. Re-pulling full state here
        // means a missed update (tab backgrounded, brief network blip)
        // self-corrects instead of leaving this client stuck on stale
        // votingActive/raidActive/votes until someone manually refreshes.
        if(status === 'SUBSCRIBED') loadInitialState();
      });
  }
  ...
  function uploadAvatar(seatId, file){
    var ext = (file.name && file.name.indexOf('.') !== -1) ? file.name.split('.').pop() : 'jpg';
    var path = seatId + '-' + Date.now() + '.' + ext;

    client.storage.from('avatars').upload(path, file, { upsert: true, contentType: file.type || 'image/jpeg' })
      .then(function(res){
        if(res.error){
          console.error('[Bunker] avatar upload failed, falling back to inline base64:', res.error);
          fallbackInlineAvatar(seatId, file);
          return;
        }
        var urlRes = client.storage.from('avatars').getPublicUrl(path);
        var url = urlRes && urlRes.data && urlRes.data.publicUrl;
        if(url){
          pushSeatChange(seatId, { avatarDataUrl: url });
        } else {
          fallbackInlineAvatar(seatId, file);
        }
      });
  ...
  function fallbackInlineAvatar(seatId, file){
    var reader = new FileReader();
    reader.onload = function(e){ pushSeatChange(seatId, { avatarDataUrl: e.target.result }); };
  ...
  function setupRoomChannel(){
    ...
    roomChannel.on('broadcast', { event: 'chat' }, function(msg){
      if(msg.payload && msg.payload.text) window.BunkerUI.appendChatMessage(msg.payload.text);
    });
  ...
  function broadcastChat(text){
    if(!roomChannel) return;
    roomChannel.send({ type: 'broadcast', event: 'chat', payload: { text: text } });
  }
