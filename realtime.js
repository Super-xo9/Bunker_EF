/*
  ...
  ВАЖНО — чинит баг "слетает аватарка при любом другом изменении карточки":
    alter table seats replica identity full;

  Без этой строки Postgres по умолчанию не включает в уведомление об
  изменении строки значения больших (TOAST) колонок, если они не менялись
  именно в этом обновлении — а base64-картинка аватарки как раз такая.
  Итог: правишь профессию/жмёшь исключить — по realtime всем приходит
  строка, где aватарка выглядит как null, хотя в базе она на месте, и
  клиент из-за этого сбрасывает её на заглушку. REPLICA IDENTITY FULL
  заставляет Postgres всегда слать полную строку, включая неизменившиеся
  большие поля.
*/
(function(){
