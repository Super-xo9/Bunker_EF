...
(function(){
  ...
  var knownPeerIds = {}; // Выносим наверх, чтобы кнопка могла их очистить
  ...
  // Логика: Всегда создаем соединения, даже если камера выключена
  ...
  function setupRoomChannel(){
    ...
    roomChannel.on('presence', { event: 'sync' }, function(){
      ...
      seatByUser = updated;
      // ВАЖНО: теперь соединения создаются всегда, независимо от cameraActive
  ...
  function startCamera(seatId){
    // video only — запрашиваем ТОЛЬКО видео
    navigator.mediaDevices.getUserMedia({ video: true })
      .then(function(stream){
        ...
        // Принудительно отправляем оффер всем, чтобы все увидели видео
  ...
  function stopCamera(seatId){
    ...
    // Очищаем knownPeerIds, чтобы при включении камеры заново отправить офферы
    Object.keys(knownPeerIds).forEach(function(uid){ delete knownPeerIds[uid]; });
    ...
    // ВАЖНО: Чтобы вы продолжали видеть камеры других, нужно переустановить соединения
    if(roomChannel) {
      roomChannel.unsubscribe().then(function(){
        setupRoomChannel();
      });
    }
  }
  ...
  window.BunkerSync = {
    ...
    refreshStatus: function(){
      ...
      // Полностью пересоздаем все соединения
      Object.keys(peers).forEach(function(uid){
        try { peers[uid].close(); } catch(e) {}
        delete peers[uid];
      });
